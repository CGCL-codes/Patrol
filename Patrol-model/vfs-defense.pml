        ...
        :: sub_idx <= subdir_num ->
            ...
            if
                :: match == true -> 
                    ...
                    // our defense
                    if  
                        :: nd.nd_mnt_ns.id == 0 && dtree.dentries[nd.dentry_t].d_mntns.id != nd.nd_mnt_ns.id->
                            nd.nd_mnt_ns.id = dtree.dentries[nd.dentry_t].d_mntns.id;
                            nd.nd_mnt_ns.vfs_root = dtree.dentries[nd.dentry_t].d_mntns.vfs_root;
                        :: else
                    fi;
                    break;
                :: else ->
...
inline trailing_symlink(nd, pathstring, position) 
{
    ...
    if 
        :: pathstring.pathname[0].string == 0 ->
            nd.dentry_t = nd.nd_mnt_ns.vfs_root;
        :: else ->
            ...
            if 
                ...
                :: else ->
                    ...
                    // our defense
                    if 
                        :: dtree.dentries[nd.dentry_t].d_mntns.id != nd.nd_mnt_ns.id ->
                            nd.dentry_t = nd.nd_mnt_ns.vfs_root;
                        :: else
                    fi;
            fi;
...
inline complete_walk(task, nd, flag, returnval, complete_err) 
{
    ...
    // our design
    if 
        :: task.mnt_ns.id == dtree.dentries[nd.dentry_t].d_mntns.id ->
            goto end_complete;
        :: else ->
            if 
                :: task.mnt_ns.id == 0 && flag == FMODE_EXEC ->
                    goto access_control;
                :: else
            fi;
...
inline path_lookup(task, pathstring, flag, returnval, look_err)
{
    ...
    if 
        :: pathstring.length == -1 -> 
            goto end_lookup;
        :: else ->
            nd.nd_mnt_ns.id = task.mnt_ns.id;
            nd.nd_mnt_ns.vfs_root = task.mnt_ns.vfs_root;
            nd.done = false;
    fi;
...
inline create_host_task()
{
    ...
    host_t.mnt_ns.vfs_root = host_t.root;
}
...
inline create_container_task(rootpath)
{
    ...
    container_t.mnt_ns.vfs_root = container_t.root;
}
